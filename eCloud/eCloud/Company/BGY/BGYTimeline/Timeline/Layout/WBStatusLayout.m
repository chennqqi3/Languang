...
- (void)_layout {
    ...
    // 如果评论超过3条
    if (_status.commentsCount > 3)
    {
        _height += kWBCommentBtnWidth;
    }
    
    
    // 如果有人点赞
    if (_status.attitudesCount > 0)
    {
        _height += kWBLikeViewHeight;
    }
...
- (void)_layoutText {
    ...
    // 评论高亮状态的背景
    YYTextBorder *highlightBorder1 = [YYTextBorder new];
    highlightBorder1.insets = UIEdgeInsetsMake(-2, 0, -2, 0);
    highlightBorder1.cornerRadius = 3;
    highlightBorder1.fillColor = [UIColor colorWithWhite:0.9 alpha:1];
    
    
    
    NSArray *nameArr = @[@"用户1",@"用户2"];
    NSArray *commentArr =@[@"评论立刻拿起了法律去呢服了你开发年前后群反馈你去方额分别坑呢发了去年了老板特别",@"评论男篮的立法那里开放法兰克那里翻了呢了呢啊啊青年人群"];
    
    
    NSMutableAttributedString *text = [NSMutableAttributedString new];
    for (int i = 0; i < nameArr.count; i++)
    {
        NSMutableAttributedString *name = [[NSMutableAttributedString alloc] initWithString:nameArr[i]];
        NSRange range = NSMakeRange(0, name.string.length);
        
        if ([name attribute:YYTextHighlightAttributeName atIndex:range.location] == nil) {
            [name setColor:[UIAdapterUtil getDominantColor] range:range];
            
            // 高亮状态
            YYTextHighlight *highlight = [YYTextHighlight new];
            [highlight setBackgroundBorder:highlightBorder];
            // 数据信息，用于稍后用户点击
            highlight.userInfo = @{kWBLinkTopicName : @"12345"};
        ...
        NSMutableAttributedString *comment = [[NSMutableAttributedString alloc] initWithString:commentArr[i]];
        NSRange range1 = NSMakeRange(0, comment.string.length);
        
        if ([comment attribute:YYTextHighlightAttributeName atIndex:range1.location] == nil) {
            [comment setColor:[UIColor colorWithWhite:0.15 alpha:1] range:range1];
            
            // 高亮状态
            YYTextHighlight *highlight = [YYTextHighlight new];
            [highlight setBackgroundBorder:highlightBorder1];
            // 数据信息，用于稍后用户点击
            highlight.userInfo = @{kWBLinkTopicName : @"123456"};
            [comment setTextHighlight:highlight range:range1];
        }
        
        [text appendAttributedString:comment];
    ...
    container.size = CGSizeMake(kWBCommentContentWidth, HUGE);
