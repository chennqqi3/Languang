...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    // 设置圆角
    _avatarView.layer.cornerRadius = 40/2.0;
...
//                [_badgeImageView setImageWithURL:pageInfo.typeIcon placeholder:nil];
                [_badgeImageView sd_setImageWithURL:pageInfo.typeIcon placeholderImage:nil];
            } else {
...
//                [_imageView setImageWithURL:pageInfo.pagePic placeholder:nil];
                [_imageView sd_setImageWithURL:pageInfo.pagePic placeholderImage:nil];
            } else {
...
//    [_repostButton setBackgroundImage:[UIImage imageWithColor:kWBCellHighlightColor] forState:UIControlStateHighlighted];
    ...
    _sendTimeLabel = [YYLabel new];
    CGFloat LabelWidth = 60;
    CGFloat LabelLeft = 18;
    _sendTimeLabel.textColor = [UIColor colorWithWhite:0.6 alpha:1];
    [_sendTimeLabel setFont:[UIFont systemFontOfSize:13]];
    _sendTimeLabel.size = CGSizeMake(LabelWidth, self.height);
    _sendTimeLabel.left = LabelLeft;
    _sendTimeLabel.textVerticalAlignment = YYTextVerticalAlignmentCenter;
    _sendTimeLabel.displaysAsynchronously = YES;
    _sendTimeLabel.ignoreCommonProperties = YES;
    _sendTimeLabel.fadeOnHighlight = NO;
    _sendTimeLabel.fadeOnAsynchronouslyDisplay = NO;
    ...
    CGFloat btnWidth = 60;
    CGFloat btnRight = 15;
    _commentButton.size = CGSizeMake(btnWidth, self.height);
    _commentButton.right = CGFloatPixelRound(self.width)-btnRight;
    ...
    _likeButton.size = CGSizeMake(btnWidth, self.height);
    _likeButton.right = CGFloatPixelRound(self.width)-(btnWidth+btnRight);
...
//    [_repostButton addSubview:_repostImageView];
    _commentImageView = [[UIImageView alloc] initWithImage:[StringUtil getImageByResName:@"timeline_icon_comment"]];
    _commentImageView.centerY = self.height / 2;
    [_commentButton addSubview:_commentImageView];
    _likeImageView = [[UIImageView alloc] initWithImage:[StringUtil getImageByResName:@"timeline_icon_unlike"]];
...
- (UIImage *)likeImage {
    ...
    dispatch_once(&onceToken, ^{
        img = [StringUtil getImageByResName:@"timeline_icon_like"];
    });
    return img;
}
...
- (UIImage *)unlikeImage {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        img = [StringUtil getImageByResName:@"timeline_icon_unlike"];
    });
    return img;
}
...
//    [_contentView addSubview:_menuButton];
    ...
    _commentBackgroundView = [[UIView alloc] init];
    _commentBackgroundView.backgroundColor = [UIColor colorWithWhite:0.97 alpha:1];
    [_contentView addSubview:_commentBackgroundView];
    
    
    // 评论label
    _textLabel = [YYLabel new];
    _textLabel.left = kWBCommentPadding;
    _textLabel.width = kWBCommentContentWidth;
    ...
    _likesLabel.textColor = [UIAdapterUtil getDominantColor];
    _likesLabel.left = 25;
    [_likesLabel setFont:[UIFont systemFontOfSize:kWBCellTextFontRetweetSize]];
    _likesLabel.top = 5;
    _likesLabel.height = 25;
    _likesLabel.width = kWBCommentContentWidth-25;
    [_likeView addSubview:_likesLabel];
    // 点赞图标
    UIImageView *likeImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 7, 19, 19)];
    likeImageView.image = [StringUtil getImageByResName:@"timeline_icon_like"];
    [_likeView addSubview:likeImageView];
    // 分割线
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 32, kWBCommentContentWidth-5, 1)];
    line.backgroundColor = [UIColor colorWithWhite:0.91 alpha:1];
    [_likeView addSubview:line];
    
    
    // 查看更多评论
    _moreComment = [UIButton buttonWithType:(UIButtonTypeCustom)];
    [_moreComment.titleLabel setFont:[UIFont systemFontOfSize:kWBCellTextFontRetweetSize]];
    _moreComment.titleEdgeInsets = UIEdgeInsetsMake(0, -6, 0, 6);
    [_moreComment setTitleColor:[UIAdapterUtil getDominantColor] forState:(UIControlStateNormal)];
    [_moreComment setTitle:@"查看更多评论" forState:(UIControlStateNormal)];
    [_contentView addSubview:_moreComment];
...
//                                  completion:nil];
    ...
    [_profileView.avatarView sd_setImageWithURL:layout.status.user.avatarLarge placeholderImage:nil];
...
//    }];
    ...
    [_vipBackgroundView sd_setImageWithURL:picBg placeholderImage:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
    ...
    if (layout.retweetHeight > 0) {
        ...
        if (layout.retweetPicHeight > 0) {
            [self _setImageViewWithTop:_retweetTextLabel.bottom+6 isRetweet:YES];
        } else {
            [self _hideImageViews];
    ...
    // 点赞、评论的位置
    _toolbarView.top = _retweetBackgroundView.bottom-2; // _contentView.height;
    top += _toolbarView.height;
    
    // 给发送时间赋值
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"HH:mm"];
    NSString *strDate = [dateFormatter stringFromDate:_layout.status.createdAt];
    _toolbarView.sendTimeLabel.text = strDate; // @"12:30";
    
    
    
    CGFloat _commentBackgroundViewHeight = layout.textHeight;
    CGFloat _commentBackgroundViewY = top;
    
    // 赞数的人
    if (_layout.status.attitudesCount > 0)
    {
        _likeView.frame = CGRectMake(kWBCommentPadding, top, kWBCommentContentWidth, kWBLikeViewHeight);
        _likeView.hidden = NO;
        
        _likesLabel.text =  @"小张、小丽、来拿来看年拉菲哪里能否你懒懒懒地方了看";
        
        top += kWBLikeViewHeight;
        
        _commentBackgroundViewHeight += kWBLikeViewHeight;
    }
    else
    {
    ...
    // 查看更多评论
    if (_layout.status.commentsCount > 3)
    {
        _moreComment.frame = CGRectMake(kWBCommentPadding, top-9, 110, kWBCommentBtnWidth);
        
        _moreComment.hidden = NO;
        
        _commentBackgroundViewHeight += kWBCommentBtnWidth;
    }
    else
    {
    ...
    // 评论的背景颜色
    _commentBackgroundView.frame = CGRectMake(kWBCellPadding, _commentBackgroundViewY, kWBCellContentWidth, _commentBackgroundViewHeight-5);
