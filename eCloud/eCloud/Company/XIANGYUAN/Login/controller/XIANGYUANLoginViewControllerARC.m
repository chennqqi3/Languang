//
//  XIANGYUANLoginViewControllerARC.m
//  eCloud
//
...
@interface XIANGYUANLoginViewControllerARC ()<UITextFieldDelegate>
{
    NSString *email;
    NSString *password;
...
@implementation XIANGYUANLoginViewControllerARC

- (void)viewDidLoad {
    [super viewDidLoad];
    //111,155,197
//    self.view.backgroundColor = [UIColor colorWithRed:111/255.0 green:155/255.0 blue:197/255.0 alpha:1];
...
- (void)setupUI
{
    ...
    [self.saveImageView addGestureRecognizer:singleRecognizer];
    ...
    NSMutableAttributedString *attributeString1 = [[NSMutableAttributedString alloc] initWithString:@"  账号不存在"];
    [self insertImageWithAttributeString:attributeString1 WithImageName:@"xinhua_error"];
    NSMutableAttributedString *attributeString2 = [[NSMutableAttributedString alloc] initWithString:@"  密码错误"];
    ...
    self.emailTextfield.textColor = [UIColor colorWithRed:127/255.0 green:127/255.0 blue:127/255.0 alpha:1];
    self.passwordTextfield.textColor = [UIColor colorWithRed:127/255.0 green:127/255.0 blue:127/255.0 alpha:1];
    ...
    self.passwordTextfield.text = _isSavePassword ? [UserDefaults getUserPassword] : @"";
    ...
    dispatch_async(queue, ^{
        ...
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.phoneNumberWarning.text = @"账号不存在";
...
- (void)handleCmd:(NSNotification *)notification
{
    ...
    else if(cmd.cmdId == login_failure)
    {
        ...
        if (result.resultCode == RESULT_INVALIDUSER || result.resultCode == RESULT_FORBIDDENUSER){
            
            self.phoneNumberWarning.text = @"无效账号";
            self.phoneNumberWarning.hidden = NO;
