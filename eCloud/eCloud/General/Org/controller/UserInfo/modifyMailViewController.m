...
-(void) addButtonPressed:(id) sender{
    ...
    if([inputField.text compare:self.oldMail] == NSOrderedSame)
    {
        [self.navigationController popViewControllerAnimated:YES];
        //            [self dismissModalViewControllerAnimated:YES];
    }
    else
    {
        ...
        if(![[conn getConn]modifyUserInfo:14 andNewValue:inputField.text]) //修改邮箱
        {
...
- (void)handleCmd:(NSNotification *)notification
{
	...
	switch (cmd.cmdId)
	{
        ...
        case modify_userinfo_success:
        {
            ...
            [[eCloudDAO getDatabase] updateUserMail:inputField.text :[userid intValue]];
...
{
    UIAlertView *tempAlert=[[UIAlertView alloc]initWithTitle:[StringUtil getAlertTitle] message:[StringUtil getLocalizableString:@"email_address_too_long"] delegate:nil cancelButtonTitle:[StringUtil getLocalizableString:@"confirm"] otherButtonTitles: nil];
    [tempAlert show];
    [tempAlert release];
}
