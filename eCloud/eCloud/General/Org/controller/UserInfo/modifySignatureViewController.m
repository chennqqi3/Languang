...
@interface modifySignatureViewController ()
...
@synthesize oldSignature = _oldSignature;
...
- (void)viewDidLoad
{
    ...
    inputField.placeholder=@"最多可输入15个字";
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    {
        if ([StringUtil lenghtWithString:inputField.text ]> SIGN_MAXLEN)
        {
            return NO;
        }
        else
...
//        }
        ...
        if([inputField.text compare:self.oldSignature] == NSOrderedSame)
        {
			[self.navigationController popViewControllerAnimated:YES];
//            [self dismissModalViewControllerAnimated:YES];
        }
        else
        {
            ...
            if(![[conn getConn]modifyUserInfo:8 andNewValue:inputField.text]) //修改签名
            {
...
-(void)viewWillAppear:(BOOL)animated
{
	...
	inputField.text = self.oldSignature;
    [inputField becomeFirstResponder];
    //监听输入框消息
...
- (void)handleCmd:(NSNotification *)notification
{
	...
	switch (cmd.cmdId)
	{
        ...
        case modify_userinfo_success:
        {
            ...
            [[eCloudDAO getDatabase] updateUserSignature:inputField.text :[userid intValue]];
...
-(void)showAlterIfSignTooLong
{
    UIAlertView *temp = [[UIAlertView alloc] initWithTitle:[StringUtil getAlertTitle] message:[StringUtil getLocalizableString:@"sign_too_long"] delegate:nil cancelButtonTitle:[StringUtil getLocalizableString:@"confirm"] otherButtonTitles:nil, nil];
    [temp show];
    [temp release];
}
@end
