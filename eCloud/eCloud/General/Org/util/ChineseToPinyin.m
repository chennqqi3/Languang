...
+ (NSDictionary *)getPinyinFromString:(NSString *)string
{
    ...
    {
        [mDic setValue:@"" forKey:pinyin_all_with_space];
        [mDic setValue:@"" forKey:pinyin_all];
        [mDic setValue:@"" forKey:pinyin_simple];
    }
    else
    {
        ...
        int  nCode;
        NSMutableString *strValue = [NSMutableString stringWithString:@""];
        
        //	获取姓名拼音的首字母
        NSMutableString *firstLetterStr = [NSMutableString stringWithString:@""];
        
        //	增加中间加了空格的拼音
        NSMutableString *spacePinYin = [NSMutableString stringWithString:@""];
        ...
        for (int i=0; i< iLen; i++)
        {
            ...
            {
                //                不是中文字符
                NSString *_str = [NSString stringWithFormat:@"%c", gb2312_string[i] > 95 ? gb2312_string[i]-32 : gb2312_string[i]];
                [strValue appendString:_str];
                continue;
            }
            ...
            if([strRes length] > 0)
            {
                [strValue appendString:strRes];
                [firstLetterStr appendString:[strRes substringToIndex:1]];
                if (spacePinYin.length > 0) {
                    [spacePinYin appendString:@" "];
                }
                [spacePinYin appendString:strRes];
            }
            
            i++;
        }
        //            如果没有中文字符，那么spacePinyin长度为0，
        if (spacePinYin.length == 0) {
            spacePinYin = strValue;
            strValue = [NSMutableString stringWithString:@""];
        }
        //        带了空格的拼音
        [mDic setValue:[spacePinYin lowercaseString]  forKey:pinyin_all_with_space];
        //        全拼
        [mDic setValue:[strValue lowercaseString] forKey:pinyin_all];
        //        简拼
        [mDic setValue:[firstLetterStr lowercaseString] forKey:pinyin_simple];
    }
    [pool release];
    return mDic;
}
...
+ (NSString *) pinyinFromChiniseString:(NSString *)string
{
    ...
    for (int i=0; i< iLen; i++)
    {
		...
		if([strRes length] > 0)
		{
			...
			firstLetterStr = [firstLetterStr stringByAppendingString:[strRes substringToIndex:1]];
			[spacePinYin appendString:strRes];
			[spacePinYin appendString:@" "];
		}
    ...
    //	全拼和首字母
	if(firstLetterStr.length > 0)
		strValue = [strValue stringByAppendingString:[NSString stringWithFormat:@"_%@",firstLetterStr]];
    //	带空格的全拼 + 全拼和首字母
	[spacePinYin appendString:strValue];
	
	return [[[NSString alloc] initWithString:[spacePinYin lowercaseString]] autorelease];
}
